...
class AbstractUrbanExtractionDataset(torch.utils.data.Dataset):
    ...
    def _get_sentinel1_data(self, site, patch_id):
        ...
        for i in range(img.shape[-1]):
            file = self.root_path / site / 'sentinel1' / f'sentinel1_{site}_{patch_id}_{i}.tif'
            SD_img, _, _ = geofiles.read_tif(file)
            SD_upsampled = upsample(torch.Tensor(SD_img).permute(2,0,1)).permute(1,2,0)
            img = np.concatenate((img, SD_upsampled), axis=-1)
    ...
    def _get_sentinel2_data(self, site, patch_id):
        ...
        upsample = transforms.Resize((256, 256), interpolation=transforms.InterpolationMode.BILINEAR)
        for i in range(img.shape[-1]):
            file = self.root_path / site / 'sentinel2' / f'sentinel2_{site}_{patch_id}_{i}.tif'
            SD_img, _, _ = geofiles.read_tif(file)
            SD_upsampled = upsample(torch.Tensor(SD_img).permute(2,0,1)).permute(1,2,0)
            img = np.concatenate((img, SD_upsampled), axis=-1)
        return np.nan_to_num(img).astype(np.float32), transform, crs
...
class AbstractSpaceNet7Dataset(torch.utils.data.Dataset):
    ...
    def _get_sentinel1_data(self, aoi_id):
        ...
        img, transform, crs = geofiles.read_tif(file)
        img = img[:, :, self.s1_indices]
        upsample = transforms.Resize((img.shape[0], img.shape[1]), interpolation=transforms.InterpolationMode.BILINEAR)
        for i in range(img.shape[-1]):
            file = self.root_path / 'sentinel1' / f'sentinel1_{aoi_id}_{i}.tif'
            SD_img, _, _ = geofiles.read_tif(file)
            SD_upsampled = upsample(torch.Tensor(SD_img).permute(2,0,1)).permute(1,2,0)
            img = np.concatenate((img, SD_upsampled), axis=-1)
        return np.nan_to_num(img).astype(np.float32), transform, crs
    ...
    def _get_sentinel2_data(self, aoi_id):
        ...
        upsample = transforms.Resize((img.shape[0], img.shape[1]), interpolation=transforms.InterpolationMode.BILINEAR)
        for i in range(img.shape[-1]):
            file = self.root_path / 'sentinel2' / f'sentinel2_{aoi_id}_{i}.tif'
            SD_img, _, _ = geofiles.read_tif(file)
            SD_upsampled = upsample(torch.Tensor(SD_img).permute(2,0,1)).permute(1,2,0)
            img = np.concatenate((img, SD_upsampled), axis=-1)
        return np.nan_to_num(img).astype(np.float32), transform, crs
