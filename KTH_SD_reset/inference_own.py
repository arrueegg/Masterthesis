...
for i, batch in enumerate(tqdm(dataloader)):
    ...
    #save predictions
    directory = f"../GM12_GUM/{site}/predictions_KTH_SD/"
    if not os.path.exists(directory):
        os.makedirs(directory)

    geofiles.write_tif(Path(f"../GM12_GUM/{site}/predictions_KTH_SD/prediction_fusion_{patch_id}.tif"), y_pred_fusion.astype(np.float32), geotransform, crs)
    geofiles.write_tif(Path(f"../GM12_GUM/{site}/predictions_KTH_SD/prediction_SAR_{patch_id}.tif"), y_pred_SAR.astype(np.float32), geotransform, crs)
    geofiles.write_tif(Path(f"../GM12_GUM/{site}/predictions_KTH_SD/prediction_optical_{patch_id}.tif"), y_pred_OPT.astype(np.float32), geotransform, crs)
